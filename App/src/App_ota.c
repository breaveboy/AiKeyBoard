...
typedef struct {
    ...
    uint8_t page_buffer[BSP_FLASH_PAGE_SIZE];
...
static bool App_ota_validate_info(const AppOtaImageInfo_t *info)
{
    ...
    crc = App_ota_crc32_memory(
        info,
        offsetof(AppOtaImageInfo_t, metadata_crc32));
...
{
    return bsp_flash_erase(BSP_FLASH_REGION_INFO,
                           0U,
                           BSP_FLASH_SECTOR_SIZE) == BSP_FLASH_OK;
}
...
static bool App_ota_write_image_info(void)
{
    ...
    info.metadata_crc32 = App_ota_crc32_memory(
        &info,
        offsetof(AppOtaImageInfo_t, metadata_crc32));
    ...
    return bsp_flash_write(BSP_FLASH_REGION_INFO,
                           0U,
                           &info,
                           sizeof(info)) == BSP_FLASH_OK;
}
...
void App_ota_init(void)
{
    ...
    if (App_ota_read_image_info(&info)) {
        ...
        memcpy(g_ota.version_text,
               info.version_text,
               sizeof(g_ota.version_text));
    }
}
...
static OtaStatus_t App_ota_begin(const uint8_t *payload)
{
    ...
    if (bsp_flash_erase(BSP_FLASH_REGION_DOWNLOAD,
                        0U,
                        erase_size) != BSP_FLASH_OK) {
        App_ota_reset_context();
        g_ota.state = OTA_STATE_ERROR;
        return OTA_STATUS_FLASH_ERROR;
    }
...
static OtaStatus_t App_ota_flush_page(void)
{
    ...
    if (bsp_flash_write(BSP_FLASH_REGION_DOWNLOAD,
                        page_offset,
                        g_ota.page_buffer,
                        g_ota.page_length) != BSP_FLASH_OK) {
        g_ota.state = OTA_STATE_ERROR;
        return OTA_STATUS_FLASH_ERROR;
    }
...
static OtaStatus_t App_ota_data(const uint8_t *payload,
                                uint8_t payload_length)
{
    ...
    if ((uint32_t)data_length >
        (g_ota.image_size - g_ota.received_size)) {
        return OTA_STATUS_INVALID_IMAGE;
    }

    while (data_length > 0U) {
        uint16_t free_space =
            (uint16_t)(BSP_FLASH_PAGE_SIZE - g_ota.page_length);
        ...
        memcpy(&g_ota.page_buffer[g_ota.page_length],
               packet_data,
               copy_length);

        g_ota.page_length += copy_length;
        g_ota.received_size += copy_length;
        packet_data += copy_length;
