...
typedef struct
{
   ...
   vu32 RESERVED2;         
   vu32 OPTR1;            /*!< FLASH OPTR1 Register,               Address offset: 0x20  */
   vu32 OPTR2;            /*!< FLASH OPTR2 Register,               Address offset: 0x24  */
   vu32 RESERVED3;         
   vu32 BANK0_WRPR;       /*!< FLASH BANK0_WRPR Register,          Address offset: 0x2C  */
   vu32 BANK1_WRPR;       /*!< FLASH BANK1_WRPR Register,          Address offset: 0x30  */
   vu32 PCROP0SR;         /*!< FLASH PCROP0SR Register,            Address offset: 0x34  */
   vu32 PCROP0ER;         /*!< FLASH PCROP0ER Register,            Address offset: 0x38  */
   vu32 PCROP1SR;         /*!< FLASH PCROP1SR Register,            Address offset: 0x3C  */
   vu32 PCROP1ER;         /*!< FLASH PCROP1ER Register,            Address offset: 0x40  */
} FLASH_TypeDef;
...
#define OTP_BASE              (0x1FFF1080UL)

#define FLAHS_SIZE_ADDR       (0x1FFF1340)
#define FLASH_BANK_SIZE       ((((M32(FLAHS_SIZE_ADDR) & 0x3) + 1) * 8 * 4 * 1024) / 2)
...
int Init(unsigned long adr, unsigned long clk, unsigned long fnc)
{
    ...
    while ((FLASH->SR & FLASH_SR_BSY0) || (FLASH->SR & FLASH_SR_BSY1));       /* Check FLASH_SR_BSY */
...
int ProgramPage(unsigned long adr, unsigned long sz, unsigned char *buf)
{
    u32 optr1, optr2, wrpr0, wrpr1, pcrop0sr, pcrop0er, pcrop1sr, pcrop1er;
    ...
    wrpr0    = M16(buf + 0x08);
    wrpr1    = M16(buf + 0x0C);
    pcrop0sr = M16(buf + 0x10);
    pcrop0er = M16(buf + 0x14);
    pcrop1sr = M16(buf + 0x18);
    pcrop1er = M16(buf + 0x1C);

    FLASH->SR |= FLASH_SR_EOP; /* Reset FLASH_EOP */

    FLASH->OPTR1 = (optr1 & 0x0000FFFF);
    FLASH->OPTR2 = (optr2 & 0x0000FFFF);
    FLASH->BANK0_WRPR = (wrpr0 & 0x0000FFFF);
    FLASH->BANK1_WRPR = (wrpr1 & 0x0000FFFF);
    FLASH->PCROP0SR = (pcrop0sr & 0x0000FFFF);
    FLASH->PCROP0ER = (pcrop0er & 0x0000FFFF);
    FLASH->PCROP1SR = (pcrop1sr & 0x0000FFFF);
    FLASH->PCROP1ER = (pcrop1er & 0x0000FFFF);

    FLASH->CR |= FLASH_CR_OPTSTRT;
    FLASH->CR |= FLASH_CR_EOPIE;

    M32(0x1FFF1100) = 0xFFFFFFFF;
    __asm("DSB");
