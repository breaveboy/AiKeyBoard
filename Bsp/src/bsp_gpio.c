...
static const GPIO_TypeDef* row_ports[MATRIX_ROWS_COUNT] = {
    ROW0_PORT, ROW1_PORT, ROW2_PORT, ROW3_PORT, ROW4_PORT
};
...
void bsp_key_matrix_row_init(void)
{
    ...
    GPIO_Init.Pin   = ROW0_PIN;
    HAL_GPIO_Init(ROW0_PORT, &GPIO_Init);

    GPIO_Init.Pin   = ROW1_PIN | ROW2_PIN | ROW3_PIN | ROW4_PIN;
    HAL_GPIO_Init(ROW1_PORT, &GPIO_Init);
...
{
    HAL_GPIO_WritePin(ROW0_PORT, ROW0_PIN, state);
    HAL_GPIO_WritePin(ROW1_PORT, ROW1_PIN | ROW2_PIN | ROW3_PIN | ROW4_PIN, state);
}
