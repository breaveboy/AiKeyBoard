...
static void adc_dma_init(void)
{
    ...
    HdmaCh1.Init.Direction           = DMA_PERIPH_TO_MEMORY;  // 内存到外设
    HdmaCh1.Init.PeriphInc           = DMA_PINC_DISABLE;       // 外设地址不增
    HdmaCh1.Init.MemInc              = DMA_MINC_ENABLE;        // 内存地址增加
    HdmaCh1.Init.PeriphDataAlignment = DMA_PDATAALIGN_WORD;    // 32为对齐
    ...
    for (int i = 0; i < ADC_CHANNELS_COUNT; i++) {
        ...
        sConfig.Rank    = ADC_REGULAR_RANK_1 + i;  // 第 1 到 第 14 顺序位
